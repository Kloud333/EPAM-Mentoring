...
<div class="navigation">
    ...
    <?php for ($i = 0; $i < $pages; $i++) { ?>
        <a href="<?= (!isset($cabinet) ? \app\core\createUrl('posts') : \app\core\createUrl('user_cabinet_page', ['cabinet' => 'cabinet'])) . $link . 'page=' . ($i + 1) ?>"><?php print ($i + 1) ?></a>
    <?php } ?>
    <?php if ($page != intval($pages)) { ?>
        <a href="<?= (!isset($cabinet) ? \app\core\createUrl('posts') : \app\core\createUrl('user_cabinet_page', ['cabinet' => 'cabinet'])) . $link ?>page=<?php print intval($pages) ?>">Last</a>
    <?php } ?>
</div>
